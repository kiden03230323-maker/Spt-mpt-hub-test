--[[
╔═══════════════════════════════════════════════════════════╗
║  POWER TYCOON HUB - FINAL MASTER EDITION                 ║
║  Full Game Features + Key + Manage + Chat + Ban System   ║
║  Made for exo_blox                                       ║
═══════════════════════════════════════════════════════════╝
]]
...
local function safeRead(p) if isfile and readfile and isfile(p) then local o,d=pcall(readfile,p) if o then return d end end return nil end
local function safeWrite(p,d) if writefile then pcall(writefile,p,d) end end
local function loadJSON(p) local r=safeRead(p) if r then local o,d=pcall(HttpService.JSONDecode,HttpService,r) if o then return d end end return nil end
local function saveJSON(p,d) local o,e=pcall(HttpService.JSONEncode,HttpService,d) if o then safeWrite(p,e) end end
...
local function checkIfBanned()
    ...
    local uid = tostring(player.UserId); local did = getDeviceID()
    for _,b in ipairs(bans.banned_users or {}) do if b.userId == uid then return true, b.reason or "No reason" end end
    for _,b in ipairs(bans.banned_devices or {}) do if b.deviceId == did then return true, b.reason or "Device banned" end end
    return false, ""
end

local function banUser(uid, did, reason, banner)
    local bans = loadJSON(BAN_FILE) or {banned_users={}, banned_devices={}, ban_logs={}}
    if uid then table.insert(bans.banned_users, {userId=tostring(uid), reason=reason, banner=banner, timestamp=os.time()}); table.insert(bans.ban_logs, {userId=tostring(uid), reason=reason, banner=banner, timestamp=os.time(), action="account_ban"}) end
    if did then table.insert(bans.banned_devices, {deviceId=did, reason=reason, banner=banner, timestamp=os.time()}); table.insert(bans.ban_logs, {deviceId=did, reason=reason, banner=banner, timestamp=os.time(), action="device_ban"}) end
...
local function createKeyUI(cb)
    ...
    btn.MouseButton1Click:Connect(function()
        if inp.Text == HUB_KEY then
            saveJSON(KEY_FILE, {key=HUB_KEY, timestamp=os.time()}); st.Text="✅ Key accepted!"; st.TextColor3=Color3.fromRGB(100,255,100); task.wait(1.2); g:Destroy(); if cb then cb() end
        else st.Text="❌ Invalid key."; inp.Text="" end
    end)
    cp.MouseButton1Click:Connect(function() if setclipboard then setclipboard(HUB_KEY); st.Text="Copied!"; st.TextColor3=Color3.fromRGB(100,200,255) end end)
...
local function createLoginUI(cb)
    ...
    local sel=nil
    local function show(r) sel=r; ui.Visible=true; pi.Visible=true; lb.Visible=true; ob.BackgroundColor3=r=="owner" and Color3.fromRGB(190,140,255) or Color3.fromRGB(60,60,80); opb.BackgroundColor3=r=="operator" and Color3.fromRGB(190,140,255) or Color3.fromRGB(60,60,80); ui:CaptureFocus() end
    ...
    lb.MouseButton1Click:Connect(function()
        ...
        if v then currentUserRole=sel; st.Text="✅ Success!"; st.TextColor3=Color3.fromRGB(100,255,100); task.wait(0.6); g:Destroy(); if cb then cb(sel) end
        else st.Text=" Invalid."; ui.Text=""; pi.Text=""; ui:CaptureFocus() end
    end)
end
...
local function createManageBoard(role)
    ...
    local function refresh()
        ...
        for _,plr in ipairs(Players:GetPlayers()) do
            ...
            f.MouseButton1Click:Connect(function()
                sel=plr; for _,ff in ipairs(ul:GetChildren()) do if ff:IsA("Frame") then ff.BackgroundColor3=Color3.fromRGB(25,25,35) end end; f.BackgroundColor3=Color3.fromRGB(40,35,60)
            end)
        end
    end
    refresh()
    ...
    if role=="owner" then
        ...
        jb.MouseButton1Click:Connect(function()
            if sel and sel.Character then
                local mr=player.Character and player.Character:FindFirstChild("HumanoidRootPart"); local tr=sel.Character:FindFirstChild("HumanoidRootPart")
    ...
    elseif role=="operator" then
        ...
        wb.MouseButton1Click:Connect(function()
            if sel then
                -- Send warning via backend in real implementation
                Library:Notify({Title="Warning Sent", Description="Warned "..sel.DisplayName, Duration=3})
            end
        end)
    end
    ...
    ab.MouseButton1Click:Connect(function()
        Library:Notify({Title="Announcement", Description="Broadcast sent to all hub users.", Duration=3})
    end)
    ...
    if role=="owner" then
        ...
        sb.MouseButton1Click:Connect(function() saveJSON(MAINT_FILE, {active=true, timestamp=os.time()}); Library:Notify({Title="Maintenance", Description="Hub shutdown initiated.", Duration=3}) end)
    end
                    ...
                    for _, targetPlr in ipairs(Aura.TargetList) do
                        ...
                        if tChar and tChar:FindFirstChild("Humanoid") and tChar.Humanoid.Health > 0 then
                            ...
                            if root then
                                ...
                                if DAMAGE_REMOTE and HitAmplifier then for i=1,5 do pcall(function() DAMAGE_REMOTE:FireServer(tChar, damagePart) end) end
                                elseif DAMAGE_REMOTE then pcall(function() DAMAGE_REMOTE:FireServer(tChar, damagePart) end)
...
function startLoopbring()
    ...
    Loopbring.Connection = RunService.Heartbeat:Connect(function()
        ...
        if tChar then
            ...
            if tRoot then tRoot.CFrame = myRoot.CFrame * CFrame.new(0, 0, 2) end
        end
    end)
end
...
function startFreezeTarget()
    ...
    FreezeTarget.Connection = RunService.Heartbeat:Connect(function()
        ...
        if tChar then
            local hum = tChar:FindFirstChild("Humanoid"); if hum then hum.WalkSpeed = 0; hum.JumpPower = 0 end
            local root = tChar:FindFirstChild("HumanoidRootPart"); if root then root.Anchored = true end
        end
    end)
end
function stopFreezeTarget() 
    ...
    if FreezeTarget.Target and FreezeTarget.Target.Character then
        local hum = FreezeTarget.Target.Character:FindFirstChild("Humanoid"); if hum then hum.WalkSpeed = 16; hum.JumpPower = 50 end
        local root = FreezeTarget.Target.Character:FindFirstChild("HumanoidRootPart"); if root then root.Anchored = false end
    end
end
    ...
    for _, tool in ipairs(myChar:GetChildren()) do
        if tool:IsA("Tool") then
            for _, part in ipairs(tool:GetDescendants()) do
                if part:IsA("BasePart") then part.Size = part.Size * 3; part.Massless = true end
            end
        end
    end
end
...
function applyNoAnimation()
    local myChar = player.Character; if not myChar then return end
    local animate = myChar:FindFirstChild("Animate"); if animate then animate:Destroy() end
    local animator = myChar:FindFirstChildWhichIsA("Animator"); if animator then animator:Destroy() end
end

function applyLocalInvisible()
    local myChar = player.Character; if not myChar then return end
    for _, desc in ipairs(myChar:GetDescendants()) do
        if desc:IsA("BasePart") or desc:IsA("Decal") or desc:IsA("Texture") then desc.Transparency = 1 end
        if desc:IsA("BillboardGui") or desc:IsA("SurfaceGui") then desc.Enabled = false end
    end
end
...
function startChatSpammer()
    ...
    ChatSpammer.Connection = RunService.Heartbeat:Connect(function()
        ...
        if chatEvent and chatEvent:FindFirstChild("SayMessageRequest") then pcall(function() chatEvent.SayMessageRequest:FireServer(ChatSpammer.Message, "All") end) end
    end)
end
...
local function initializeHub()
    ...
    CombatSec:AddButton("Manage Aura Targets", function() Library:Notify({Title="Target Manager", Description="Select players in the custom overlay.", Duration=3}) end, {Title="Manage Targets", Description="Open target selection."})
    CombatSec:AddToggle("Kill Aura", false, function(state) Aura.Enabled = state; if state then startAuraLoop() else stopAuraLoop() end end, {Title="Kill Aura", Description="Automatically hits targets."})
    CombatSec:AddToggle("Instant Kill", false, function(state) InstantKill = state end, {Title="Instant Kill", Description="Brute-force kills targets."})
    ...
    ToolSec:AddToggle("Auto Get Tools", false, function(state)
        AutoGetTools = state
        if state then
    ...
    end, {Title="Auto Get Tools", Description="Grabs weapons from pads."})
    ToolSec:AddToggle("No Cooldown", false, function(state) NoCooldown = state end, {Title="No Cooldown", Description="Removes tool cooldowns."})
    ...
    MPT_Combat:AddToggle("Hit Amplifier", false, function(state) HitAmplifier = state end, {Title="Hit Amplifier", Description="Spams damage remotes."})
    
    local MPT_Control = MPT_MainPage:CreateSection("🎯 Target Control")
    MPT_Control:AddToggle("Loopbring", false, function(state) Loopbring.Enabled = state; if state then startLoopbring() else stopLoopbring() end end, {Title="Loopbring", Description="Teleports target to you."})
    MPT_Control:AddToggle("Freeze Target", false, function(state) FreezeTarget.Enabled = state; if state then startFreezeTarget() else stopFreezeTarget() end end, {Title="Freeze Target", Description="Anchors and freezes target."})
    ...
    MPT_Tools:AddToggle("Big Tools", false, function(state) BigTools = state; if state then applyBigTools() end end, {Title="Big Tools", Description="Scales tools by 3x."})
    MPT_Tools:AddToggle("Invisible", false, function(state) LocalInvisible = state; if state then applyLocalInvisible() end end, {Title="Invisible", Description="Makes you transparent."})
    MPT_Tools:AddToggle("No Animation", false, function(state) NoAnimation = state; if state then applyNoAnimation() end end, {Title="No Animation", Description="Removes character animations."})
    MPT_Tools:AddToggle("Fly Tools", false, function(state) ToolFollow.Enabled = state; if state then startToolFollow() else stopToolFollow() end end, {Title="Fly Tools", Description="Tools float and follow targets."})
    ...
    MPT_Utils:AddTextbox("Chat Message", "MPT Hub is OP!", function(text) ChatSpammer.Message = text end, {Title="Chat Message", Description="Message to spam."})
    MPT_Utils:AddToggle("Ultra Spammer", false, function(state) ChatSpammer.Enabled = state; if state then startChatSpammer() else stopChatSpammer() end end, {Title="Chat Spammer", Description="Spams chat message."})
